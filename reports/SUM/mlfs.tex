...
The fourth step consists of configuring the prepare SUT configuration file \\(\texttt{MASS\_STEPS\_LAUNCHERS/PrepareSUT.sh}, the actions provided in this file enables \MASS to collect the code coverage; within this file the following actions must be provided by the engineer (see~\ref{preparesut_single}), notice that we use the tool \texttt{bear} for the generation of the \texttt{compile\_commands.json}:
