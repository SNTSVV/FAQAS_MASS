...
After the execution of \MASS the results will be stores in dedicated folders, such organization is defined as follows:
...
\begin{enumerate}
	...
	\begin{itemize}
		\item main.csv: complete mutation traces
		\item sampled\_mutants: list of sampled mutants to be executede
		\item all\_live: list of live mutants
		\item all\_killed: list of killed mutants
		\item traces\_live: live mutants traces
		\item traces\_killed: killed mutants traces
		\item mutant folder: there is one folder for each executed mutant, this folder contains the mutant code coverage information and test case execution logs.
	\end{itemize}
