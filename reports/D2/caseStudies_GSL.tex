...
\MREVISION{C-P-47}{In the context of FAQAS, we consider both the libcsp (GSL branch) and the extension of the CSP library.}
...
\MREVISION{C-P-48}{On the other hand, the mutations to the payload of the packet will consist of applying the bit-flip (BF) operator to any of the bits of the payload data structure. The mutations to the payload should be detected by the components that uses libcsp (e.g., libparam).}
...
In the context of FAQAS, libparam will be considered for both activities code, and data driven mutation testing.}
