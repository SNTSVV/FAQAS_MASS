...
\Procedure{identifyNonRedundantMutants}{$\mathit{M},\mathit{TS},\mathit{TR},\mathit{Cov}_M$}\Comment{$M$ is a  list of mutants, $\mathit{TS}$, $\mathit{TR}$, and $\mathit{Cov}_M$ are defined above}
...
\EndIf
\EndFor
\If {$\mathit{redundant}==\mathit{FALSE}$}
\State \textbf{break} \Comment{No need to compare with all the mutants if we know that it is not redundant}
\EndIf
\EndFor
\If {$\mathit{redundant}==\mathit{FALSE}$}
...
\end{algorithmic}
\vspace{-3mm}
\caption{Algorithm for identifying non-equivalent and non-redundant mutants}
%\vspace{-0.2cm}
