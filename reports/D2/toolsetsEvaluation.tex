...
Section~\ref{subsec:background} provides a brief introduction to the main framework used by the tools assessed in our study.
Section~\ref{subsec:srciror} to Section~\ref{subsec:dextool} describe the results achieved for each of the subjects of the study.
...
\subsection{SRCIRor Evaluation Results}
\label{subsec:srciror}

\subsubsection{Overview of the subject of the study}
...
Most of the warnings are caused by SRCIRor trying to mutate memory addresses declared in C code, e.g., \texttt{0x03A6} into \texttt{(- 1)x03A6}, and unsigned integer literals (e.g., \texttt{0u}, \texttt{1u} into \texttt{(-1)u}, \texttt{1u}).
...
\subsection{Mull Evaluation Results}
\label{subsec:mull}
...
By commenting out these two functions, is it possible to execute the mutation process of the adcs\_ExecuteReactionwheelDesaturation source file. Milu produces 35 mutants belonging to the adcs\_ExecuteReactionwheelDesaturation function.
However, is not possible to compile any of the 35 mutants produced by Milu. We observe errors during the ESAIL compilation process, in particular there are errors being thrown from the preprocessed area of the mutant. In Listing~\ref{miluerror}, an example of the error obtained during compilation is shown.
