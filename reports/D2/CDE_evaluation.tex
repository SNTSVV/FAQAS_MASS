...
\subsubsection{Subjects of the study}
...
The generation of mutants is fast, it takes at most \FIXME{182} seconds on the largest component (ESAIL). On average, across case studies, it takes \FIXME{11} milliseconds to generate one mutant. 
...
As expected, the largest mutation score is achieved for MLFS, whose test suite achieves MC/DC coverage. \REVNOV{PTCR-20}{In this project, based on related work, we assume a good quality test suite to have a mutation score above 75\%. However, the objective of this report is not to evaluate the quality of the provided test suites but to evaluate the approach for estimating the mutation score. In general there are a number of factors that affect the observed mutation score; for example, the presence of equivalent mutants (which makes the mutation lower than what should be). The inspection of equivalent mutants for the case studies is till ongoing, at this stage. Also, the reasons for not achieving a high mutation score need an in depth discussion. For GSL case studies, this seems to be related to a number of off-the-shelf components integrated in the source of the system that are not expected to be tested by the test suite. The mutation score for non off-the-shelf-componets will be computed in later stages of the project.}
...
We notice that for test suites that ensure MC/DC coverage (i.e., MLFS), even a very small sampling ratio (i.e., 0.01) guarantees a $\delta_{acc}$ below 5\%. However, \FIXME{to achieve an accurate mutation score estimate for all software artifacts, a minimum sampling ratio of 0.09} is required.
...
\subsubsection{RQ3 - SDL accuracy}
...
%\item Execution of the \APPR test suite with a random selection of x\% SDL mutants
\end{itemize}
...
We randomly select \FIXME{20} mutants for each bucket and inspect them with the help of engineers to determine if they are equivalent. 
We aim to select a threshold value below which equivalent mutants are detected with a precision above \FIXME{90\%}. 
%Note: In case a mutant is shared by multiple configurations we may assume it is resampled
In total, we manually inspect \FIXME{200} mutants (20 mutants x 5 buckets x 2 configurations).
...
%Table~\ref{table:results:precision:equivalent} provides the results.
%
...
To this end, we apply \APPR to the five case study systems described in Section~\ref{sec:empirical:subjects} and compute the mutation score according to \APPR Step 8 (Section~\ref{sec:appr:score}).
 %considered in previous research questions and to the whole ESAIL. 
 %We report the mutation score obtained without removing equivalent and redundant mutants, and the mutation score obtained after removal of equivalent/redundant mutants. 
...
\REVNOV{PTCR-PABG-27}{The FSCI sampling approach with a confidence interval of 10\% is the most accurate approach for the estimation of the mutation score. Also, combined with test suite reduction and prioritization it saves, on average more than 80\% of the execution time; in other words, the mutation testing process takes only 20\% of the time required if all the mutants are executed with all the test cases that cover them. Similarly, the number of test cases being executed is reduced by more than 80\%.}
