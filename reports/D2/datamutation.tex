...
Since in space software most of the communication between software components is performed thanks to an API provided by a dedicated communication layer, in FAQAS mutation probes are integrated in the software API functions that either serialize or deserialize the data being sent on the communication channel.

Mutation probes are integrated in the software under test by engineers who manually add specific function calls in the source code of the software under test. Since communication APIs vary from system to system, it is not possible to define a tool that automatically modifies the source code or the executable code of the SUT.  Instead, we aim to provide a data-driven mutation testing API that enable to mutate common data structures in C/C++ software (e.g., variables or type int, float, double, or C++ std lib vectors).
...
The activity \EMPH{Mutate Data} is driven by a \INDEX{fault model} that specifies the set of mutation operators to apply~\cite{di2015generating}. 
...
During data mutation the test suite is executed multiple times. The number of times to execute depends on a stopping criterion that depend on the choice made by the engineer to (1) exercise each test case against a data fault class (hereafter, test coverage stopping criterion), or (2) 
exercise each data fault class (hereafter, fault coverage stopping criterion) or (3) sample the mutations to apply against a certain test case (hereafter, sampling stopping criterion).
