...
\RQ{} The activity \emph{Randomly sort mutants} indicates that  \emph{FAQAS-CompileAndExecuteMutants} generates a randomly prioritized list of mutants to compile and execute from the \emph{unique mutants csv}.
...
\RQ{} The activity \emph{Select and remove first mutant} indicates that  \emph{FAQAS-CompileAndExecuteMutants} selects the first mutant in \emph{sorted list of mutants} and remove it from the list.
...
\RQ{} The program \emph{FAQAS-CompileAndExecute} compiles and execute mutants till a termination criterion is met. The termination criterion depends on the mutants selection strategy:
\begin{itemize}
