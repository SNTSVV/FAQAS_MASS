...
This section discusses first the results of a preliminary feasibility study for the automated generation of test cases based on mutation analysis results (Section~\ref{sec:testSuiteAugmentation:feasibility}); then, it presents the solution developed by FAQAS, which is called SEMuS (Section~\ref{sec:semus:all}).

\subsection{Feasibility Study}
\label{sec:testSuiteAugmentation:feasibility}
...
Riener et al.~\cite{riener2011test}, instead, rely on a \INDEX{bounded model checker} (BMC).
...
\input{listings/GSLaugmentation2.tex}

\input{klee}

\subsection{FAQAS Test Suite Augmentation Tool (SEMuS)}
\label{sec:semus:all}
