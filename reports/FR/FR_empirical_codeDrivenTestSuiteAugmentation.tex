...
\TODO{Add LibUtil and ESAIL Unit}
...
\subsection{Example shortcomings identified with SEMuS}
\label{sec:shortcoming:semus}

\subsubsection{ASN.1: lack of oracles}
...
We confirmed this shortcoming with ASN.1 engineers, who fixed this issue in their source code.





\subsubsection{LibUtil: faulty implementation}

In the case of libutil we have discovered a buffer underflow.
Listing~\ref{test_code_util} shows the test case generated by SEMuS.
It invokes function \emph{timestamp\_diff} (i.e., the function under test), with the parameters \emph{base} and \emph{diff} set as follows:
\begin{verbatim}
base = {tv_sec = 0, tv_nsec = 0};
diff = {tv_sec = 0, tv_nsec = 1} 
\end{verbatim}

Function \emph{timestamp\_diff} computes the diff between \emph{base} and \emph{diff}; the result replaces the value of \emph{base}.

The output generated when executing the test case (i.e., file \emph{test1.c.expected}) is shown in Listing~\ref{test_code_util_out}. By looking at the file \emph{test1.c.expected}, which reflects the actual behavior 
of the implementation, engineers can spot a fault (underflow).
Indeed, the difference between base and diff (erroneously) leads to a large integer, instead it should lead to the generation of an error code as return value.


\begin{lstlisting}[style=CStyle, caption=LibUtil test code template generated by SEMuS., label=test_code_util]
flag T_INT_enc_dec(const T_INT* pVal, int* pErrCode, const char* filename)
{
        (void)argc;
    (void)argv;
...
\begin{lstlisting}[style=CStyle, caption=Output for test case in Listing~\ref{test_code_util}., label=test_code_util_out]
flag T_INT_enc_dec(const T_INT* pVal, int* pErrCode, const char* filename)
result_faqas_semu = 0
