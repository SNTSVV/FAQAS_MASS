...
% \TODO{The list not of interest should go to the previous chapter}
...
\TODO{Rephase the sentence below}
...
\TODO{What is the 'final mutation score'? We should define how it is computed.}
...
In the following sections, we provide the list of test cases that covers the 30 mutants killed for the sources \texttt{clock.c}, \texttt{timestamp.c}, \texttt{memory.c}, and \texttt{error.c}. For each test case, we also provide the oracle with the expected values for the function. Note that engineers are supposed to verify if the expected output is correct. In the case that the expected output is not correct, it means that the software is faulty.

\TODO{I would report the in an appendix. ALso, we should report both the test case and the oracles.}
\TODO{You should explicitely indicate that engineers are supposed to verify if teh expected output is correct, if not, it means that the software is faulty}.
\TODO{Did you check if the same input kills other mutants? We shall do it, so that we minimize the number of test cases reported}
\TODO{What about the mutants for which SEMuS do not generate inputs but terminate? I would suggest to consider those mutants as likely equivalent and list them as such}
...
\lstinputlisting{generated_test_cases/direct-src.drivers.sys.memory/long_to_string/FAQAS_SEMu-out/produced-unittests/test000006.ktest.c.expected}
...
\lstinputlisting{generated_test_cases/direct-src.drivers.sys.memory/long_to_string/FAQAS_SEMu-out/produced-unittests/test000016.ktest.c.expected}
...
\lstinputlisting{generated_test_cases/direct-src.drivers.sys.memory/long_to_string/FAQAS_SEMu-out/produced-unittests/test000020.ktest.c.expected}
...
\lstinputlisting{generated_test_cases/direct-src.drivers.sys.memory/long_to_string/FAQAS_SEMu-out/produced-unittests/test000064.ktest.c.expected}
...
\lstinputlisting{generated_test_cases/direct-src.drivers.sys.memory/long_to_string/FAQAS_SEMu-out/produced-unittests/test000078.ktest.c.expected}
...
\lstinputlisting{generated_test_cases/direct-src.drivers.sys.memory/long_to_string/FAQAS_SEMu-out/produced-unittests/test000087.ktest.c.expected}
...
\lstinputlisting{generated_test_cases/direct-src.drivers.sys.memory/long_to_string/FAQAS_SEMu-out/produced-unittests/test000093.ktest.c.expected}
