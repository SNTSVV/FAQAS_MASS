...
\EMPH{Note}{It has been shown that test suites with mutation score greater than 75\% have a higher fault detection rate~\cite{chekam2017empirical}. This implies that the Libutil test suite has a high fault detection rate for the statements that are covered (i.e., 60.2\%). Please note that statement coverage adequacy is required for the mutation score to be representative of a high fault detection rate.}
...
\EMPH{Action Item 3}{It seems that you are not testing the execution of stop when the monitor is not running.}
...
\EMPH{Action Item 4}{This mutant shows that you do not verify the result returned by the function when the \texttt{day} field of the date contains a value higher than 31.}
...
But the assertion fails, because the original GSL code returns a \texttt{GS\_OK} code. Is this a bug or an expected behaviour?
}
