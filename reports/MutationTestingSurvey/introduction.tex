...
Unfortunately, the software testing procedures enforced by international standards heavily rely on the capability of software engineers to define test suites that extensively exercise the software. \EMPH{Automated and effective methods to evaluate the quality of test suites are thus necessary.} 
...
This book provides a detailed \EMPH{overview of  the mutation testing process} and how it can be applied in the context of space and embedded systems. 
\MREVISION{C10}{Despite the proposed solutions can be applied to different kinds of software for space systems (i.e., ground desktop software, ground embedded software, on-board embedded software), in this book, the term \INDEX{space software} is used to refer to on-board embedded software.}
This book also describes the \EMPH{common limitations} of mutation testing and \EMPH{countermeasures} to these limitations; finally, it provides a description of the approaches that can be applied to automatically \EMPH{generate test cases} to achieve high mutation coverage.

We focus on two major families of mutation approaches~\cite{natella2016assessing}: (1) \EMPH{code-driven mutation testing approaches} that alter the software code to mimic programming errors, (2) \EMPH{data-driven mutation techniques} that alter runtime data to simulate failures in communicating components. Code-driven techniques have shown to be an effective means to evaluate the effectiveness of test suites~\cite{Andrews06} while data-driven mutation techniques can identify complex boundary cases not handled by the developed software~\cite{di2015evolutionary}.
