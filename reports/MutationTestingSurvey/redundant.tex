...
Previous studies showed redundancies between mutation operators and proved that a certain subset of operators is sufficient to measure test effectiveness. To this end, the \INDEX{sufficient set of operators} has been applied (see Section~\ref{sec:sub:sufficient}). 
...
For example, the \INDEX{sufficient set} proposed by Namin et al.~\cite{siami2008sufficient} reduced redundant mutants in a 93\% on C programs. 

%In this direction, Namin et al.~\cite{siami2008sufficient} proposed a statistical analysis procedure for identifying a subset of operators that could predict mutation score, their approach reduced mutants in a 93\% on C programs. 
