...
%Fabrizio: your sentence is a little convoluted. It seems that you apply different optimizations to the original program with the aim to recreate the same executable code of the mutant.
%If can be considered, since every time the original program can be transformed by an optimisation to one of its mutants, then the mutant can be defined as equivalent. 
...
%Fabrizio: I cannot understand the following sentence. Based on what is written, it seems they are simply checking if the mutated code is reachable. In practice they check if there exist one input that exercise the mutant, not if teh mutant is equivalent to the original program.
The main idea is to analyze the path condition of a mutant, in which the mutant is defined equivalent if and only if the input constraint is unsatisfiable. 
...
%Fabrizio: not necessaty, it is also strange
%In like manner, 
...
%Fabrizio: what it means that "a fitness function that sets a poor fitness value to an equivalent mutant" Does it mean that they defined a fitness functiona that enable t spot an equivalent mutant? How this is done?
...
%Fabrizio: try to minimize the use of passive form
...
%Fabrizio: please double check that what we write is correct. It sounds strange that we use statement coverage as a replacement for control flow
...
%Fabrizio: teh following sentence is too convoluted
%On the empirical evaluation performed on JAXEN (12,449 LOC), by analyzing the mutants that were covered by the test bench but not killed, they performed several experiments with two main results. 
%Fabrizio: you should say what is Jaxen in parenhesis "(a <adjective> software consisting of about 12\,449 LOC)"
Two are the main results achieved in an empirical evaluation performed with JAXEN (12,449 LOC). 
...
%Fabrizio: do you mean "above zero"
...
%Second, the relation between the impact on control-flow degree and likelihood of the mutant of being non-equivalent, the results were obtained by analyzing two specific subsets (e.g., (a) subset with the 20 mutants with highest impact, and (b) subset with the 20 mutants with lower impact), 
%from the first subset with higher impact
Second, 90\% of the 20 mutants with the highest impact  
%Fabrizio: you meant lowest but positive (i.e., non zero), right?
are non-equivalent, while 55\% of the 20 mutants with the lowest positive impact are equivalent. 
...
%Fabrizio: you use "e.g.," when it is an example out of many possible; you use "i.e." when it is the full set
%anyway here we should use a sentence non between parenthesis otherwise we would have nested parenthesis
...
%You mean " both change code and data coverage" or "either code or data coverage" ?
Schuler et al. \cite{schuler2010covering,schuler2013covering} discovered that mutants that show a different code and data coverage with respect to the original program, have a likelihood of 68\%-79\% to be non-equivalent.
...
%Fabrizio: you should indicate which are theres
than operators that only change data (57\%).
%Fabrizio: what is a false negative in this case? A mutant indicated to be non-equivalent while being equivalent?
Finally, 
 authors show that by measuring the impact on both data and code coverage it is possible to obtain few false negatives (61\%). 
 %Fabrizio: do they use one statement coverage metric or many?
Data and code coverage can be captured by metrics that focus on differences in statement (i.e., number of methods that have at least one statement that is executed at a different frequency between mutated and original program) and data (i.e., number of methods that have at least one different return value between mutated and original program) coverage. 
...
%Fabrizio: is this what you actually meant?
...
ignore
%avoid 
%Fabrizio: is "corrupted" the term they used? should we add "corrupted data (i.e., data generated by the mutated code)"
mutants that fail to propagate corrupted data 
...
%Fabrizio: "Consequently" is used when there is a logical cosequence, here we have just a choice
%Consequently, 
...
%Fabrizio: "On the other hand" with respect to what?
%On the other hand, 
%Fabrizio: no need to put author names, this is not a survey
...
%Fabrizio: not sure it is english, check if I got the meaning
...
%Fabrizio: at this point you never said that they rely on patterns.
