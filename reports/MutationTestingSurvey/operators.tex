...
The purpose of the sufficient set is to enable the definition of test suite that kill mutants generated with a broader set of operators. In other words, 
test suites that kill mutants generated by the sufficient set should kill mutants generated by broader set of operators.} 
%\DONE{Is "deletion statement" or "statement deletion" ? OSCAR: Statement deletion}
The sufficient set was then updated by Andrews et al.~\cite{andrews2005mutation} with the statement deletion operator (SDL). The SDL operator has been included because it ensures that every pointer-manipulation and field-assignment statement is properly tested, thus targeting faults not checked by the rest of the sufficient operators. In addition, recent research result show that the SDL operator is the most effective for fault detection~\cite{delamaro2014designing}. 
...
\MREVISION{C11.b}{Table~\ref{table:codeoperatorssummary} provides the number of operators we collected for each category and the context where they can be applied. The table also provides a strategy to prioritize the mutation operators for space-context software. In particular, in the last column \textit{Relevance for FAQAS} we prioritized the categories according to the effectiveness and ease of implementation. 
Based on this criterion, we selected \INDEX{deletion operators} as the most relevant, since according to the literature they form a cost-effective alternative to other operators in terms of number of mutants produced and equivalent mutants~\cite{delamaro2014designing}. 
