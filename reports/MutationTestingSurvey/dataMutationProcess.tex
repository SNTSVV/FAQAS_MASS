...
It is based on a fault model capturing the type of data fault that might affect the system. The fault model is produced by software engineers based on their domain knowledge and experience~\cite{di2015generating}. \MREVISION{C18}{The considered faults might be due to programming errors, hardware problems, or critical situations in the environment (e.g., noise in the channel).}
...
For example, consider the case in which the components of a system work with two different data types but the test suite exercises only one data type. Objective O1 might be fully achieved if we observe failures for all the mutations applied to the data type being exercised. However will not be able to apply any of the operators for the other type of data and thus objective O2 enables us to determine that the test suite is inadequate.
When data-driven mutation testing is performed against components that are expected to ensure robustness, objective O2 ensures also that the components robustness is properly tested.}
