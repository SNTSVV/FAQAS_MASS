...
Although \emph{pFuzzer} addresses the problem of redundant mutant by systematically generating valid inputs that cover diverse sets of lexical and syntactical features, it may still produce equivalent mutants.
...
Compared with a Model-Based Blackbox Fuzzer (MoBF) approach, the \emph{MoWF} prototype was able to expose all of 13 vulnerabilities on an empirical evaluation carried on nine subject programs, while the MoBF approach only detected 6 out of 13 vulnerabilities. The paper indirectly addresses the problem of redundant mutants by performing a systematic path exploration search covering critical locations in the source code (i.e., by well covering the space of possible inputs).
