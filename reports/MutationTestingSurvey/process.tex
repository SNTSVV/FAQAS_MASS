...
\index{Test Suite Evaluation Process}
The Test Suite Evaluation process concerns the automatic generation of modified versions (i.e., the mutants) of the software under test (SUT) and the evaluation of the quality of the SUT test suite. 
...
\TODO{Does SSDL delete a statement or a line? PLease be consistent in the following}

\TODO{Complete the fllowing}
%F: non c'era un nome migliore rispetto a 'mutationtestingexample' ?
Listings X to Y, provide the results obtained wen applying the \textit{SSDL} operator to the source code in Listing \ref{mutationtestingexample}.
Listing \ref{mutationtestingexample} shows the implementation of function \texttt{isPalindrome}, which ....
Listings X show that line X has been deleted, ...
Listing X+1...
...
%F You tend to have a very verbose writing. YOu should go straight to th epoint. IN Fugure XX we present
...
\TODO{You should write "the function <functionName>" not "the <functionName> function"}

In Listing \ref{mutantts} we introduce an example test suite for the function \texttt{isPalindrome}, which has been introduced in Listing~\ref{mutationtestingexample}. 
The outcome of the execution of the test suite against the function \texttt{isPalindrome}  is that all the four test cases pass.
\TODO{are passing => pass}
Instead, if now the same test suite is executed against the mutant \textit{M1}, we observe that again all the test cases are passing. This means that the existing test suite is not adequate, since a test suite should identify all the faults introduced.

\TODO{The example is not good enough. You should show that the test suite fails with most of the mutants except one. Please reorder the examples and the description to fix it.}
...
In a real world system, unfortunately, the activity \emph{create mutants} may lead to a high number of mutants to be generated, which, in turn, leads to scalability issues due to the compilation and execution of the different program versions generated. Recent surveys provide an overview of existing optimization techniques~\cite{ferrari2018systematic};
...
\TODO{We should have an example of an equivalent mutant. Can you think of anything for the isPalindrome? Maybe replacing l++ with ++l ? If so you have to introduce the operator as well.}
...
\TODO{COplete}
In the examples concerning the mutation of function \textit{isPalindrome}, mutants X Y Z are killed, instead X mutants are live.



The identification of killed and live mutants enable the definition of a mutation adequacy criterion as follow, \emph{a test suite is mutation-adequate if all mutants are killed by at least one test of the test suite}. 
...
\TODO{Add comment concerning the example.}
...
The identification of test inputs has the objective of identifying inputs for the SUT that make the SUT produce an output that is different than the one produced by one of the mutants not killed by the existing test suite. The generation of test oracles, instead, should lead to the generation of executable code instructions (e.g., assertions) that verify if the output generated by the SUT is correct.

%F: already done, deleted statement => statement deleted}
In the case of the example of function \textit{isPalindrome}, test suite augmentation should lead to introducing a new test case that exercises the  statement deleted by the operator. %Particularly, 
More precisely, we should introduce a test case that exercises a non-palindrome string. In Listing \ref{mutantts_improved}, we added a test case with the input \textit{abbaa}, this test case will fail against \textit{M1}. This process should be done iteratively for all the live mutants of Table \ref{table:mutants}. 
\TODO{If there are multiple live mutant, please add an example for each}
...
\TODO{Please STOP using the word 'besides' :)}
