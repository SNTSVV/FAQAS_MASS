...
\INDEX{Random mutation testing} consists of selecting randomly a certain percentage of the generated mutants. So, instead of executing all mutants only a reduced percentage is evaluated.
...
they show that sampling mutants uniformly across different program elements (i.e., to have a same percentage of mutants selected for every function/methods) leads to a more accurate mutation score prediction than randomly sampling mutants globally or uniformly distribute the sampled mutants across different mutation operators (i.e., to have a same number of mutant for each mutation operator.}
...
In a different direction, Delamaro et al. \cite{delamaro2014designing,delamaro2014experimental} experimented with mutants that involve deletion operations (\INDEX{statements deletion}) and found that they form a cost-effective alternative to other operators, and selective mutation strategies, as it was found that they produce significantly less equivalent mutants (8.5\% in comparison to the average of 11.9\% during experimentation). 
...
For \INDEX{test case prioritization}, the idea is to sort the test cases in order to kill the mutants as early as possible. For instance, Just et al. \cite{just2012using} proposed to use execute first those test cases with shorter runtime for time saving.
