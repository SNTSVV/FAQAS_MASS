...
\TODO{The following sentence needs to be clarified.}
...
\TODO{In the following, I cannot undestand what is in parenthesis}
...
\TODO{No change to do here. However please keep them in mind for the current work.}
...
\TODO{In the following, we miss an indication of how they measure if a mutant is close. Also, we do not say what performs better.}
Zhang et al. \cite{zhang2013faster} proposed two approaches for sorting the test cases; the first is based on the code coverage, it prioritizes test cases that execute the mutated statement more times because they have a higher probability to kill the mutant, the second approach prioritizes test cases that kill mutants whose mutated statement is close to the mutated statement of the mutant under test.
