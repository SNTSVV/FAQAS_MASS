...
\begin{enumerate}
	...
	Table~\ref{table:libgscsp_coverage} provides code coverage information of the libgscsp integration test suite for the GSL extension to the CSP library. Below, we report the files covered by the test suite and targeted by mutation analysis; they correspond to 56\% of the source files. %(10/18)
...
Below, we report the files covered by the test suite and targeted by mutation analysis; they correspond to 45\% of the source files. %(32/71)
...
In order to evaluate the feasibility of code-driven mutation analysis for libgscsp, we conducted a preliminary experiment using the mutation operators AOR, ROR, ICR, LCR, ABS, UOI and SDL we generated 6\,196 mutants. For the experimentation we targeted only the libcsp (GSL branch) source code. Preliminary results can be found in Table~\ref{table:libgscsp_preliminary}.
...
The data-driven mutation analysis process in libgscsp will target the data packet transferred between the server and the client using the CSP protocol. Specifically, the mutations will affect the header of the packet, which contains routing information (see Figure~\ref{fig:csp_packet}) and the payload 
...
\MREVISION{C-P-48}{
%On the other hand, the mutations to the payload of the packet will consist of applying the bit-flip (BF) operator to any of the bits of the payload data structure. 
The mutations to the payload is expected to be performed within the the components that uses libcsp (e.g., libparam); at that level it would be feasible to understand the data format.}
...
Below, we report the files covered by the test suite and targeted by mutation analysis; they correspond to 82\% of the source files. %(55/67)
...
%
%In order to analyze the feasibility of the code-driven mutation analysis, we conducted a preliminary experimentation using the mutation operators AOR, ROR, ICR, LCR, and SDL we generated 16\,886 mutants. Preliminary results can be found in Table~\ref{table:libutil_preliminary}.
