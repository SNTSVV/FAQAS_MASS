...
%	\TODO{In GlobalCoverageMatrix parli di line. Si tratta di line o statement?}
%	\DONE{I files gcov mostrano tutte le line del codice sorgente; rappresentano gli statement con il numero/frequenza, ed invece le line che non sono statement gli rappresenta con un '-'. Quindi si, si trattano di lines.}
	
	\REVISION{P-1b}{
	\begin{itemize}
		\item \textit{GlobalCoverageMatrix}: csv file containing one row per source file under test. Each row in GlobalCoverageMatrix includes the name of the source file followed by a list with the coverage frequency for all the lines within that source file. Source lines that cannot be covered (e.g., code comments) are represented by a '-' character. For example, the coverage row:

		\texttt{source.c:-,-,-,1,4,4,-,0,1,0} 
		
		Indicates that the source \texttt{source.c} contains 10 lines, where the first three lines cannot be covered (e.g., comments), the fourth line has been covered one time, and the fifth line has been covered four times.
		...
		\item \textit{TestTimeouts}: csv file containing one row per test case. Each row includes the name  of the test case, followed by the test execution time threshold, in seconds. For example, the row \texttt{acos:6} indicates that the test case \texttt{acos} cannot be executed for more than six seconds.
	\end{itemize}
...
\REVISION{P-1a}{All the programs that automate the different steps of \MASS communicate only through the files that had been described in Section~\ref{sec:component:design}.
