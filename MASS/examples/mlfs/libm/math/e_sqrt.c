 ...
 *      4000117691/16/NL/FE
 * \brief Implementation file for the sqrt procedure.
...
#endif
{
	...
	ix0 += ix0 + (__int32_t)((ix1&(__uint32_t)sign)>>31U);
	ix1 += ix1;
	...
	while(r!=0) {
		...
		s1  = t1+r;
		if((((__int32_t)t1&sign)==sign)&&((__int32_t)s1&sign)==0) s0 += 1;
		ix0 -= t;
		if (ix1 < t1) ix0 -= 1;
		ix1 -= t1;
		q1  += r;
