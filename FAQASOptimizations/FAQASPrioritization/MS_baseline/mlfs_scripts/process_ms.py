...
def get_prioritized_tests(prioritization_path, mutant_name, mutant_line):
    tests_list = []

    relative_path = os.path.join(prioritization_path)
    try:
...
def process_prioritized_ms(traces, prioritization_path, test_path, original_order, mutant_dict_kl, iteration):
    ...
    for key, value in traces.items():
        ...
        p_tests = get_prioritized_tests(prioritization_path, mutant_name, mutant_line)
