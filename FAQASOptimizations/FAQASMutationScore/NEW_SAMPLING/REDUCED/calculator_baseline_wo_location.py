...
def get_prioritized_tests(prioritization_path, mutant_name, mutant_line, iteration):
    tests_list = []

    iteration_str = "sources_" + str(iteration)

    relative_path = os.path.join(prioritization_path, iteration_str)
    try:
        ...
        print(mutant_name, mutant_line)
                                                                                                                                     
    return tests_list

def define_killed(file_path, prioritization_path, test_path, original_order, iteration):
    ...
    for key, value in traces.items():
        ...
        p_tests = get_prioritized_tests(prioritization_path, mutant_name, mutant_line, iteration)
        status = 0
...
if __name__ == '__main__':
    ...
    for i in range(1, 10 +1):
        ...
        print(sim_file)
        results.append(define_killed(sim_file, prioritization_path, test_path, original_order, i))
