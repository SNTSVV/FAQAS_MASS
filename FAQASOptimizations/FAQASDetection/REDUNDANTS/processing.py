...
def retrieve_traces_mutant(mutant_key, traces, common_tests):
    traces_mutant_test = []
    for trace in traces[mutant_key]:
        executed_test = trace.split(';')[3].replace("/home/gsl/libutil/tst/", "")
...
def process(mutants, coverages):
    ...
    for combination in combinations:
        ...
        if len(cov_traces_a) == 0 or len(cov_traces_b) == 0:
            print("not redundant: no common covered tests")
            continue
        ...
        if len(tests_a) == 0 and len(tests_b) == 0 and line_a == line_b:
            print("redundants: no coverage for a nor b")
            continue
        elif len(tests_a) == 0 and len(tests_b) == 0 and line_a != line_b:
            print("not redundants: no common covered tests")
            continue
        ...
        mut_traces_a = retrieve_traces_mutant(combination[0], mutants_traces_dict, common_tests)
        mut_traces_b = retrieve_traces_mutant(combination[1], mutants_traces_dict, common_tests)
        
        if not common_killed_tests(mut_traces_a, mut_traces_b):
            print("not redundant: no common killed tests")
            continue
