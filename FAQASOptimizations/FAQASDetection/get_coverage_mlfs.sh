...
for i in $(find $SRC_MUTANTS -name '*.c');do
    ...
    if grep -Fxq $mutant_name $MUTANT_LIST;then
        ...
        exec_loc=$(find $MUTANTS_TRACES -name 'main.csv' | xargs grep --with-filename -m 1 "${mutant_name}" | awk -F':' '{print $1}' | xargs dirname)          
        ...
        for tst in */;do
            ...
            RESULTS_COV=${MUTANT_COVERAGE_FOLDER}/${tst}Reports/Coverage/Data/coverage.txt
