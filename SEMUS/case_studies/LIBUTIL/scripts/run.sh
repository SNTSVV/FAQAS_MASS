...
echo "# Running for source file $ENV_FAQAS_SEMU_SRC_FILE..."
...
produce_unittest()
{
    ...
    sed -i 's|$in_file|$repo_root_dir/src/linux/drivers/sys/memory.c $in_file|' $unittest_outdir/runtest.sh
...
if [ $phase -le 1 ]; then
    ...
    echo "$repo_rootdir/$ENV_FAQAS_SEMU_SRC_FILE:$alllines" > $output_topdir/.srciror/coverage
