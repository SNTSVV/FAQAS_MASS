...
#        if ! $build_bc_func $f_path $f_path.tmp > /dev/null 2>&1
        if ! $build_bc_func $f_path $f_path.tmp
        then
...
produce_unittest()
{
    ...
    # patching for ESAIL VM
    for f in `ls $unittest_outdir/*.c`; do
        ktest_file=$(basename -- $f)
        ktest_name=${ktest_file%.ktest.c}
        sed -i'' '/#include/d' $f
        sed -i'' '/(void)arg/d' $f
        sed -i'' "s/main(int argc, char\*\* argv)/$ktest_name(void)/" $f
    done

    func=$(basename -- $func_template_path | cut -d'.' -f1)
    final_test_file=$unittest_outdir/${func}_unittest.c
    test -f $final_test_file && >$final_test_file

    includes="
    #ifdef TARGET\n
    #include <rtems.h>\n
    int main(void);\n
    #include \"_Ext/rtems_init.c\"\n
    #endif\n

    #include <stdio.h>\n
    #include <stdlib.h>\n
    "
    echo -e $includes >> $final_test_file
    
    pushd $repo_rootdir/Source > /dev/null 
    src=$(echo ${ENV_FAQAS_SEMU_SRC_FILE%.c} | xargs basename)
    for h in $(find . -name "*$src*.h");do
        echo "#include \"$h\"" >> $final_test_file
    done
    popd > /dev/null
    
    
    for f in `ls $unittest_outdir/*ktest.c`; do
        cat $f >> $final_test_file
    done
    ...
    \n
    "
    echo -e $header >> $final_test_file

    for f in `ls $unittest_outdir/*.c`; do
        [[ "$f" == *"unittest.c" ]] && continue
        ...
        test_text="
            ...
            printf(\"Executing test $ktest_name...\\n\");
            $ktest_name();
            printf(\"*********\\n\");
        
        "
        echo $test_text >> $final_test_file           
    done
