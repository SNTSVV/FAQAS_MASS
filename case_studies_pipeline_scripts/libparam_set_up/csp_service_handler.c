    ...
    //end of the mutation probe
            ...
            while (param_pos < param_max) {
               ...
               /* Get data */
                ...
                v_REPLY[0] = (unsigned long long int) reply->action;
                v_REPLY[1] = (unsigned long long int) reply->table_id;
                v_REPLY[2] = (unsigned long long int) reply->length;
                v_REPLY[3] = (unsigned long long int) reply->checksum;
                v_REPLY[4] = (unsigned long long int) reply->seq;
                v_REPLY[5] = (unsigned long long int) reply->total;
                v_REPLY[6]= (unsigned long long int) reply->payload.copy.from;
                v_REPLY[7]= (unsigned long long int) reply->payload.copy.to;

                mutate_FM_REPLY(v_REPLY);

                reply->action = (uint8_t) v_REPLY[0];
                reply->table_id = (uint8_t) v_REPLY[1];
                reply->length = (uint16_t) v_REPLY[2];
                reply->checksum = (uint16_t) v_REPLY[3];
                reply->seq = (uint16_t) v_REPLY[4];
                reply->total = (uint16_t) v_REPLY[5];
                reply->payload.packed[0]=(uint8_t)v_REPLY[6];
