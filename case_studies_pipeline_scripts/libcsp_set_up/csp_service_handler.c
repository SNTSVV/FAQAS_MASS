...
void csp_service_handler(csp_conn_t * conn, csp_packet_t * packet) {
	...
	// manually inserted probe Identifier
	int damat_buffer[7];

//	unsigned long int damat_buffer[0] = (unsigned long int) conn->idin.ext;
    ...
    /*
	// conn->idin.ext = ( uint_32_t ) damat_buffer[0];
	// conn->idin.pri = damat_buffer[1];
	conn->idin.src = damat_buffer[2];
	conn->idin.dst = damat_buffer[3];
	conn->idin.dport = damat_buffer[4];
	conn->idin.sport = damat_buffer[5];
	conn->idin.flags = damat_buffer[6];
    */
