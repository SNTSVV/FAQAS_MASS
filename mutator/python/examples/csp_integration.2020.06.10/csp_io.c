...
int mutate( unsigned int *v, FaultModel *fm ){
...
int csp_send_direct(csp_id_t idout, csp_packet_t * packet, const csp_route_t * ifroute, uint32_t timeout) {
	...
	packet->id.ext = idout.ext;        
        ...
        unsigned int v[6] = {packet->id.pri, packet->id.src ,packet->id.dst, packet->id.dport, packet->id.sport, packet->id.flags};
        ...
        csp_log_packet("OSCAR ANTES Fl 0x%02X, Sp %u, Dp %u, D %u, Src %u, Pri %u",
 packet->id.flags, packet->id.sport, packet->id.dport, packet->id.dst, packet->id.src, packet->id.pri);
...
csp_log_packet("OSCAR DESPUES Fl 0x%02X, Sp %u, Dp %u, D %u, Src %u, Pri %u",                                                       
 packet->id.flags, packet->id.sport, packet->id.dport, packet->id.dst, packet->id.src, packet->id.pri);
#endif  
