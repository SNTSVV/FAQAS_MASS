...
#include "FAQAS_dataDrivenMutator.h" //FAQAS INCLUSION
...
::Smp::Bool PdhuPdoService::IndicationReceived(::Smp::UInt8 functionCode, const std::vector<Smp::UInt8>& data)
{
    ...
    Smp::UInt8 commandId = data[0]; //data IS THE INPUT VECTOR
    std::vector<Smp::UInt8> newData(8); //newData IS THE BUFFER WE TARGET WITH THE MUTATIONS
    newData[0] = data[0];
...
// AnsData 	Byte 1.7 	Up to 7 Bytes 	The Answer payload. The size and the meaning of the payload depend on the command ID
    ...
    case 0x01: //RMTCMD(O1,01) STO_SND_FRM
    {
    ...
    case 0x04: //RMTCMD(O1,04) STO_GET_HEAD
    {
        ...
        if(statusCode == RCSC_CmdOk)
        {
            ...
            FaultModel *fm = _FAQAS_STO_GET_HEAD_FM();
            mutate( &newData, fm );
            _FAQAS_delete_FM(fm);
    ...
    break;
    case 0x05: //RMTCMD(O1,05) STO_GET_TAIL
    {
        ::Smp::UInt64 sequenceId;
        ...
        if(statusCode == RCSC_CmdOk)
        {
            ...
            FaultModel *fm = _FAQAS_STO_GET_TAIL_FM();
            mutate( &newData, fm );
            _FAQAS_delete_FM(fm);
    ...
    break;
    case 0x1C: //RMTCMD(O1,1C) CRC_COMPUTE
    {
        ...
        FaultModel *fm = _FAQAS_CRC_COMPUTE_FM();
        mutate( &newData, fm );
        _FAQAS_delete_FM(fm);
